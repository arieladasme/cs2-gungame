...
- [ ] **Developer Portal de `GKS Bot`** (no se puede por API): desactivar "Public Bot"; en OAuth2 → Redirects agregar `https://arieladasme.github.io/gks/vincular/`; generar el Client Secret y ponerlo en `DiscordClientSecret` del `GGExtras.json` del server. Sin eso, `/vincular` solo ofrece el código para el juego.
