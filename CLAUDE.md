...
- [ ] **Activar `gks.goadatti.com`**: en Vercel (proyecto `gks` → Settings → Domains) agregar el subdominio y en el DNS de Squarespace crear el CNAME `gks` que indique Vercel. Después cambiar el link de `#bienvenida` de `gks-gold.vercel.app` al dominio.
