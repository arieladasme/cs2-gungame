...
- ✅ F4 sonidos (cerrado 2026-07-16): addon `gungame_sounds` compilado por CLI (`resourcecompiler`) desde `F:\git\gungame-sounds-addon\` y subido al Workshop — ID **3766168370**. 17 MP3 + 14 soundevents `gg.*` (Winner con 5 pistas aleatorias nativas). Montado con MultiAddonManager (`mm_extra_addons`/`mm_client_extra_addons`) + `UseSoundEvents: true` en `gungame.json`. Probados in-game. Causa raíz del volumen raro que tenía el intento anterior: el comando `play` usa el canal `snd_toolvolume` (no respeta el mixer del juego); los soundevents sí lo respetan.
...
- ✅ Stack actualizado (2026-09-11): CSS **1.0.374** / Metamod **git1411** / CS2 build **25218825** / MultiAddonManager **v1.5.4**. Plugin recompilado; sin regresiones en el log. Ojo con el corte de SourceHook API 017/018 — CLAUDE.md §4.
- 🔶 F5 pool: 3 stock (`ar_pool_day` w2, `ar_shoots` w2, `ar_baggage` w1) + 3 Workshop (`fy_iceworld` 3070238628, `fy_snow` 3592238209, `aim_map` 3070549948) en `GGMCmaps.json`. `RememberPlayedMaps: 1`. **Validado 2026-09-11: los 3 Workshop NO cargan en el server local** — sin GSLT el server no está logueado en Steam y el mapa entra en loop de recarga de ~1 s. Los stock funcionan. Flujo completo sigue pendiente.
